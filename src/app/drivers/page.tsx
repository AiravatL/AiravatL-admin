...
export default function DriversPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'created' | 'bids' | 'wins'>('created')
  const [error, setError] = useState<string | null>(null)

  const vehicleTypes = ['three_wheeler', 'pickup_truck', 'mini_truck', 'medium_truck', 'large_truck']
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">Monitor and manage driver accounts and activities</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">{stats?.totalBidsPlaced || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Auctions Won</p>
              <p className="text-3xl font-bold text-gray-900">{stats?.totalAuctionsWon || 0}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Vehicle Distribution</h2>
          <div className="space-y-3">
            {Object.entries(stats?.vehicleTypeDistribution || {}).map(([type, count]) => (
              <div key={type} className="flex items-center justify-between">
                <div className="flex items-center">
                  <Car className="w-4 h-4 text-gray-400 mr-2" />
                  <span className="text-gray-700">{formatVehicleType(type)}</span>
                </div>
                <div className="flex items-center">
                  <div className="w-24 bg-gray-200 rounded-full h-2 mr-3">
                    <div 
                      className="bg-indigo-600 h-2 rounded-full" 
                      style={{ width: `${((count / (stats?.totalDrivers || 1)) * 100)}%` }}
                    ></div>
                  </div>
        ...
        {stats?.topDrivers && stats.topDrivers.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Top Performing Drivers</h2>
            <div className="space-y-3">
              {stats.topDrivers.map((driver, index) => (
                <div key={driver.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-gray-900">{driver.auctions_won} wins</p>
                    <p className="text-sm text-gray-500">{driver.bids_count} bids</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <tbody>
              {filteredDrivers.map((driver) => (
                <tr key={driver.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-4 px-4">
                    <div>
                      <p className="font-medium text-gray-900">{driver.vehicle_number || 'N/A'}</p>
                      <p className="text-sm text-gray-500">
                        {driver.vehicle_type ? formatVehicleType(driver.vehicle_type) : 'Not specified'}
                      </p>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="text-center">
                      <p className="font-semibold text-gray-900">{driver.bids_count || 0} bids</p>
                      <p className="text-sm text-gray-500">{driver.auctions_won || 0} wins</p>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                    <div className="text-center">
                      <p className="font-semibold text-gray-900">
                        {driver.bids_count > 0 
                          ? Math.round(((driver.auctions_won || 0) / driver.bids_count) * 100)
                          : 0}%
                      </p>
