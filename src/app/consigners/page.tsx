...
export default function ConsignersPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'created' | 'auctions'>('created')
  ...
  const fetchConsignerData = async () => {
    try {
      ...
      if (auctionStatsResult.error) {
          ...
          consignersData.map(async (consigner) => {
            const [auctionsResult, bidsResult] = await Promise.all([
              ...
              supabase
                ...
                .select('id')
                .in('auction_id', 
                  (await supabase.from('auctions').select('id').eq('created_by', consigner.id)).data?.map(a => a.id) || []
                )
            ])
            ...
            return {
              ...
              completed_auction_count: auctions.filter(a => a.status === 'completed').length,
        ...
        const activeConsigners = consignersWithStats.filter(c => c.auction_count > 0).length
        const totalAuctionsCreated = consignersWithStats.reduce((sum, c) => sum + c.auction_count, 0)
        const completedAuctions = consignersWithStats.reduce((sum, c) => sum + c.completed_auction_count, 0)
        const avgAuctionsPerConsigner = totalConsigners > 0 ? totalAuctionsCreated / totalConsigners : 0
  ...
  const filterConsigners = () => {
    ...
    // Search filter
    if (searchTerm) {
      filtered = filtered.filter(consigner =>
        consigner.username?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        consigner.first_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        consigner.last_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-2">Monitor and manage consigner accounts and activities</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Total Consigners</p>
              <p className="text-3xl font-bold text-gray-900">{stats?.totalConsigners || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Active Consigners</p>
              <p className="text-3xl font-bold text-gray-900">{stats?.activeConsigners || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900">{stats?.totalAuctionsCreated || 0}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600 mb-1">Avg per Consigner</p>
              <p className="text-3xl font-bold text-gray-900">
                {stats?.avgAuctionsPerConsigner ? Math.round(stats.avgAuctionsPerConsigner * 10) / 10 : 0}
              </p>
            </div>
      ...
      {stats?.topConsigners && stats.topConsigners.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Top Consigners</h2>
          <div className="space-y-3">
            {stats.topConsigners.map((consigner, index) => (
              <div key={consigner.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="font-medium text-gray-900">
                      {consigner.first_name || consigner.last_name 
                        ? `${consigner.first_name || ''} ${consigner.last_name || ''}`.trim()
                        : consigner.username}
                    </p>
                ...
                <div className="text-right">
                  <p className="font-semibold text-gray-900">{consigner.auction_count} auctions</p>
                  <p className="text-sm text-gray-500">{consigner.completed_auction_count} completed</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 font-semibold text-gray-900">Auctions</th>
            ...
            <tbody>
              {filteredConsigners.map((consigner) => (
                <tr key={consigner.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-4 px-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">@{consigner.username}</p>
                    </div>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="text-center">
                      <p className="font-semibold text-gray-900">{consigner.auction_count || 0}</p>
                    ...
                    <div className="text-center">
                      <p className="font-semibold text-gray-900">
                        {consigner.auction_count > 0 
                          ? Math.round(((consigner.completed_auction_count || 0) / consigner.auction_count) * 100)
                          : 0}%
                      </p>
          ...
          {filteredConsigners.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">No consigners found matching your search.</p>
            </div>
          )}
        </div>
      </div>
